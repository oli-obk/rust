...
impl<'tcx> ImplTraitHeader<'tcx> {
    ...
    pub fn is_fully_generic_for_reflection(self) -> bool {
        ...
        let trait_ref = self.trait_ref.instantiate_identity();
        let self_ty = trait_ref.self_ty().visit_with(&mut ParamFinder::default()).is_continue();
        // We use two separate finders, as generic params may be used twice, once in the trait and once
        // in the self type. But never twice in the self type, or twice in the trait.
        let mut finder = ParamFinder::default();
        self_ty && trait_ref.args[1..].iter().all(|arg| arg.visit_with(&mut finder).is_continue())
    }
}
