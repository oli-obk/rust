...
impl Cursor<'_> {
    ...
    pub fn advance_token(&mut self) -> Token {
        ...
        let token_kind = match first_char {
            ...
            '#' if matches!(self.first(), '"' | '#') => self.guarded_double_quoted_string(),
    ...
    fn guarded_double_quoted_string(&mut self) -> TokenKind {
        ...
        if self.first() == '"' {
            ...
            if let Ok(n) = u8::try_from(n_start_hashes) {
                self.eat_literal_suffix();

                Literal {
                    kind: GuardedStr {
                        n_start_hashes: NonZeroU8::new(n),
                        // Always succeeds because `n_end_hashes <= n_start_hashes`
                        n_end_hashes: n_end_hashes.try_into().unwrap(),
                    },
                    suffix_start,
                }
            } else {
                Literal { kind: GuardedStr { n_start_hashes: None, n_end_hashes: 0 }, suffix_start }
            }
        } else {
